...
That uses [`phpunit.external-apis.xml`](../phpunit.external-apis.xml): `APP_ENV=development`, default `CONFIG_PATH=config/airports.json.example` (overridable: parent process sets `CONFIG_PATH` before PHPUnit because it is not `force="true"`), and `RUN_EXTERNAL_UPSTREAM_TESTS=1`. The OpenWeatherMap tile test **skips** unless your `CONFIG_PATH` file includes a real `openweathermap_api_key` (for example point `CONFIG_PATH` at a maintainer secrets file when running manually).
