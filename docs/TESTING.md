...
| [`.github/workflows/production-health-check.yml`](../.github/workflows/production-health-check.yml) | Daily HTTPS probes against production (`scripts/production-health-check.php`; ~1 rps pacing, one `/v1/airports` fetch, three random airports for `/weather` and `/webcams` only, JSON shape checks via `lib/production-health-check-evaluators.php`), then `check-builtin-aviation-links.php`. Requires outbound network; not part of `make test-ci`. |
...
**`MetricsInternalHealthEndpointsIntegrationTest` (`tests/Integration/MetricsInternalHealthEndpointsIntegrationTest.php`):** Exercises `health/variant-health-flush.php`, `health/status-bundle-mirror-refresh.php`, and the matching `*_via_http()` helpers in `lib/metrics.php`. Endpoints only allow `REMOTE_ADDR` loopback; curling from the host to a published Docker port often yields **403**, so the tests **skip** with an explanation. Run from **inside** the web container (see the file docblock) or point `TEST_API_URL` at a stack where the HTTP client is seen as localhost. If nothing is listening, cases **skip** (`http_code` 0), consistent with other Integration HTTP tests.

**Partner logo HTTP checks (`tests/Integration/PartnerLogoApiTest.php`):** Base URL is **`TEST_API_URL`**, then **`TEST_BASE_URL`**, then **`http://localhost:8080`**. `make test-local` / `make test-e2e` set `TEST_API_URL=http://localhost:9080`, so these tests hit the isolated stack when it is running. **`WeatherEndpointTest`** uses **`TEST_API_URL`** only (same default fallback). **`SmokeTest`** uses **`TEST_PROD_URL`**, then **`TEST_API_URL`**, then **`http://localhost:8080`** (optional production smoke via `TEST_PROD_URL`). If nothing is listening, every case **marks itself skipped**, so GitHub Actions jobs without a web server and plain `make test-unit` do not hit the live HTTP path. **Regression coverage without a web server** lives in `tests/Unit/PartnerLogoCachePathsTest.php` (subprocess runs `lib/partner-logo-cache.php` and `api/partner-logo.php` with `CONFIG_PATH` pointed at the fixture config).
