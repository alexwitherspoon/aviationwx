...
```
Protected Services:
- ProFTPD: one control listener inside the container (`listen_port` = `ftp_control`; FTP and explicit FTPS/STARTTLS on that port). `ftps_explicit_tls` in `network_ports` is for host UFW/fail2ban when you expose a distinct inbound port for explicit-TLS clients; it is not a second ProFTPD bind (same instance as `ftp_control`; use `ftps_alt` NAT for extra inbound ports).
...
`config.network_ports.ftps_alt` adds another host TCP port that REDIRECTs to `ftp_control` (same ProFTPD instance; client source IP preserved for logging and bans). Defaults: `ftp_control` 2121, `ftps_alt` unset. See [FTPS alternate control port (NAT redirect)](OPERATIONS.md#ftps-alternate-control-port-nat-redirect).
