...
- **Fresh Ubuntu VPS Setup**: See [PRODUCTION_DEPLOYMENT.md](docs/PRODUCTION_DEPLOYMENT.md) for step-by-step from scratch
- **Local Development**: See [LOCAL_SETUP.md](docs/LOCAL_SETUP.md) for local setup
...
- See [CONFIGURATION.md](docs/CONFIGURATION.md) for complete webcam configuration examples.
...
- Missing webcam images → confirm cron is running inside container (`docker compose exec web ps aux | grep cron`) and `/tmp/aviationwx-cache/webcams/` is writable (cache is ephemeral, cleared on reboot).
