...
Returns a GeoJSON `FeatureCollection` of airspace restriction geometries for the **airports directory map** (`pages/airports.php`). Features include properties such as `notam_id`, `banner_headline`, `status`, `status_line`, `geometry_kind` (`polygon`, `multipolygon`, or `circle`), `restriction_kind`, `official_link`, and style hints. Drawable geometry is read from the national airspace record store (`cache/notam/map-airspace.json`) with envelope `schema_version` / `merge_logic_version` / `data_updated_at` for serve eligibility (not deploy SHA). Status and map colors are projected at serve time. Overlapping polygons that share restriction kind, map style, and vertical limits may be **display-merged** into one footprint (`display_merged`, `member_notam_ids`, `member_count`); this projection does not change the store. `coverage_scope` is `faa_nms_wfs`, `faa_tfr_wfs`, or `faa_nms_side_channel` depending on healthy sources. Stale, missing, or version-mismatched store data returns empty features with `failclosed: true` and `failclosed_reason`. Successful responses use the short shared HTTP cache window (`NOTAM_API_CACHE_TTL_SECONDS`); JSON `cache_ttl_seconds` reflects the disk/client poll interval (`getNotamCacheTtlSeconds()`).
...
- **METAR/NOAA Aviation Weather** (uses hourly thresholds, not multipliers):
