...
├── admin/
│   ├── diagnostics.php       # System diagnostics (operator console only)
│   └── metrics.php           # Metrics (operator console only)
├── health/
...
- `loadConfig()` merges the aggregate into each airport as `_country_resolution_geo_iso` only when SHA matches (strict; no merge on mismatch). If the file is missing, invalid, or mismatched, merged geometry keys are cleared so APCu-backed config does not retain stale auto-geometry. Effective ISO comes from `getEffectiveIso3166Alpha2ForAirport()` in `lib/config.php` (ICAO letter-scheme fallback uses `inferIso3166Alpha2FromIcaoPrefix()` when `iso_country` is absent); built-in external links are resolved from `lib/aviation-region-links.php` using `aviationLinkRegionFromIso()` and region profiles.
...
Deduplicate by NOTAM ID
  ↓
Filter for relevance (closures by scope rules, TFRs by circle, polygon, or legacy geometry)
  ↓
Determine status (active, inactive_scheduled, upcoming_today, upcoming_future)
  ↓
...
- **Diagnostics**: System health via `ops.aviationwx.org` operator console and `/health.php`
...
- **Metrics**: `ops.aviationwx.org` operator console for monitoring
