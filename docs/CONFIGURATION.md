...
VPN connections allow secure access to private camera networks at remote airport sites. The system supports WireGuard, OpenVPN, and IPsec protocols.
...
The VPN manager automatically generates keys and assigns client IPs on first run.
...
Keys are auto-generated. Copy them from manager logs into `airports.json` to persist them.
...
For IPsec, manually set a strong PSK (32+ character random string).

### Using VPN with Webcams

Once VPN is connected, update webcam URLs to use private IPs from the remote subnet:
...
After configuring VPN, generate the client config file:

```bash
./scripts/generate-client-config.sh kspb wireguard
```

This creates `config/vpn-clients/kspb.wireguard.conf` ready to import at the remote site.

For more details, see `docs/VPN_USAGE.md`.
