...
echo "1. Check if SSL is enabled in vsftpd configs:"
echo "   docker compose -f docker/docker-compose.prod.yml exec web grep '^ssl_enable=' /etc/vsftpd/vsftpd_ipv4.conf"
echo "   docker compose -f docker/docker-compose.prod.yml exec web grep '^ssl_enable=' /etc/vsftpd/vsftpd_ipv6.conf"
echo ""

echo "2. Check certificate paths in vsftpd config:"
echo "   docker compose -f docker/docker-compose.prod.yml exec web grep '^rsa_cert_file=' /etc/vsftpd/vsftpd_ipv4.conf"
echo "   docker compose -f docker/docker-compose.prod.yml exec web grep '^rsa_private_key_file=' /etc/vsftpd/vsftpd_ipv4.conf"
echo ""
...
echo "   docker compose -f docker/docker-compose.prod.yml exec web vsftpd -olisten=NO /etc/vsftpd/vsftpd_ipv4.conf 2>&1 | head -10"
echo ""

echo "9. Check TLS version settings:"
echo "   docker compose -f docker/docker-compose.prod.yml exec web grep -E '^ssl_tlsv|^# ssl_tlsv' /etc/vsftpd/vsftpd_ipv4.conf"
echo ""

echo "10. Check if vsftpd is listening on port 2121:"
echo "    docker compose -f docker/docker-compose.prod.yml exec web netstat -tlnp | grep 2121 || docker compose -f docker/docker-compose.prod.yml exec web ss -tlnp | grep 2121"
echo ""
...
echo "12. Check full vsftpd IPv4 config for SSL-related settings:"
echo "    docker compose -f docker/docker-compose.prod.yml exec web grep -E 'ssl_|tls_' /etc/vsftpd/vsftpd_ipv4.conf | grep -v '^#'"
echo ""
...
echo "  grep \"^ssl_enable=\" /etc/vsftpd/vsftpd_ipv4.conf"
echo "  echo \"\""
echo "  echo \"=== Certificate Paths ===\""
echo "  grep \"^rsa_cert_file=\" /etc/vsftpd/vsftpd_ipv4.conf"
echo "  grep \"^rsa_private_key_file=\" /etc/vsftpd/vsftpd_ipv4.conf"
echo "  echo \"\""
...
echo "  grep -E \"^ssl_tlsv\" /etc/vsftpd/vsftpd_ipv4.conf"
echo "'"
echo ""
