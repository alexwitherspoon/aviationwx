...
/**
 * ProFTPD loopback MasqueradeAddress for on-box upload health probes (127.0.0.1 / ::1).
 * External clients still use MasqueradeAddress from the endpoint cache (mod_ifsession).
 */
function buildProftpdLoopbackPassiveConfSnippet(): string
{
    return <<<'EOF'
  <Class upload_local_client>
    From 127.0.0.0/8
  </Class>
  <Class upload_local_client_v6>
    From ::1/128
  </Class>
  <IfClass upload_local_client>
    MasqueradeAddress               127.0.0.1
  </IfClass>
  <IfClass upload_local_client_v6>
    MasqueradeAddress               ::1
  </IfClass>
...
/**
 * Wrap a MasqueradeAddress directive so it does not apply to loopback probe clients.
 */
function wrapProftpdMasqueradeAddressForRemoteClients(string $address): string
{
    return "  <IfClass !upload_local_client>\n"
        . "    <IfClass !upload_local_client_v6>\n"
        . "      MasqueradeAddress               {$address}\n"
        . "    </IfClass>\n"
        . "  </IfClass>\n";
}
...
function buildProftpdMasqueradeConf(array $endpoints): string
{
    ...
    if ($caps['ipv6'] && $ipv6 !== null && (!$caps['ipv4'] || $ipv4 === null)) {
        return "# Generated by upload-endpoints.php - do not edit by hand\n"
            . "<IfModule mod_ifsession.c>\n"
            . $loopback
            . wrapProftpdMasqueradeAddressForRemoteClients($ipv6)
            . "</IfModule>\n";
    }

    if ($caps['ipv4'] && $ipv4 !== null && $caps['ipv6'] && $ipv6 !== null) {
        $ipv4Masquerade = wrapProftpdMasqueradeAddressForRemoteClients($ipv4);
        $ipv6Masquerade = wrapProftpdMasqueradeAddressForRemoteClients($ipv6);

        return <<<EOF
# Generated by upload-endpoints.php - do not edit by hand
<IfModule mod_ifsession.c>
{$loopback}  <Class upload_ipv4_mapped>
...
{$ipv4Masquerade}  </IfClass>
  <IfClass upload_ipv4_direct>
{$ipv4Masquerade}  </IfClass>
  <IfClass !upload_ipv4_mapped>
    <IfClass !upload_ipv4_direct>
{$ipv6Masquerade}    </IfClass>
  </IfClass>
</IfModule>
...
/**
 * ProFTPD mod_tls certificate directive names for the key type in $certFile.
 *
 * @return array{0: string, 1: string} [certificateDirective, keyDirective]
 */
function resolveProftpdTlsCertificateDirectives(string $certFile): array
{
    $rsa = ['TLSRSACertificateFile', 'TLSRSACertificateKeyFile'];
    ...
    if (($details['type'] ?? null) === OPENSSL_KEYTYPE_EC) {
        return ['TLSECCertificateFile', 'TLSECCertificateKeyFile'];
    }
...
EOF;
    ...
    $cert = $certFile;
    $key = $keyFile;
    [$certDirective, $keyDirective] = resolveProftpdTlsCertificateDirectives($certFile);
