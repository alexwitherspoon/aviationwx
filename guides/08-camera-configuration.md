...
**Key limitation:** UniFi Protect does **not** support scheduled FTP uploads. Use the **AviationWX Bridge** (Guide 10) on a device on the same network as your NVR. The Bridge captures snapshots via Local RTSP and uploads to AviationWX.
...
2. Go to **Home** → **Protect** → **UniFi Devices**
3. Select your camera
4. Go to **Settings** → **Advanced**
5. Toggle the resolution you want (e.g., High)
6. Copy the URL shown (e.g., `rtsps://192.168.1.1:7441/QxJCVMefFHfBnqrp?enableSrtp`)

**Step 2: Convert to Local RTSP format**

UniFi's URL needs a few adjustments. Apply these three changes:

| Change | From | To |
|--------|------|-----|
| Protocol | `rtsps://` | `rtsp://` (remove the second `s`) |
| Port | `7441` | `7447` |
| Query string | `?enableSrtp` | Remove entirely |
