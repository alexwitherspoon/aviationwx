/**
 * Airport dashboard application code.
 *
 * Page-specific data (AIRPORT_DATA, INITIAL_WEATHER_DATA, staleness
 * thresholds, webcam seeds, etc.) is declared in an inline bootstrap
 * script emitted by pages/airport.php before this file loads. Top-level
 * const/let bindings from that classic script share the global lexical
 * environment, so they are visible here without any wiring.
 *
 * This file must stay free of PHP: it is served statically with a
 * build-hash query (?v=) and cached by the browser across visits.
 */

// Production logging removed - only log errors in console

// NOTE: the page previously declared isMobileDevice twice; the simpler
// width/touch variant (defined later in this file, near the theme code)
// always won under function hoisting. The shadowed UA-based duplicate was
// removed when this code moved out of airport.php so the file declares one
// isMobileDevice with the behavior the page has always had.
...
function daPerformanceTier(performance) {
    return performance && performance.tier ? String(performance.tier) : 'normal';
}

function daPerformanceOperationalEndLabel(performance) {
    if (!performance || !performance.best_end || typeof performance.best_end !== 'object') {
        return '';
    }
    const endId = performance.best_end.end_id ? String(performance.best_end.end_id).trim() : '';
    if (endId === '') {
        return '';
    }
    const rwyId = performance.best_end.rwy_id ? String(performance.best_end.rwy_id).trim() : '';
...
function daPerformanceSelectionBasisNote(performance) {
    if (!performance || !performance.selection_basis) {
        return '';
    }
    const endLabel = daPerformanceOperationalEndLabel(performance);
    if (endLabel === '') {
...
function daPerformanceTooltip(tier, performance) {
    ...
    const basisNote = daPerformanceSelectionBasisNote(performance);
    if (tier === 'warning') {
...
function daPerformanceAriaLabel(densityAltitudeFt, tier, performance, distUnit) {
    ...
    if (tier === 'warning') {
        return `${base}. Warning: dangerously high for average GA aircraft; verify performance numbers before flight.${daPerformanceSelectionBasisNote(performance)}`;
    }
    if (tier === 'caution') {
        return `${base}. Caution: higher than normal; verify performance numbers before flight.${daPerformanceSelectionBasisNote(performance)}`;
    }
    return base;
}
...
function formatDensityAltitudePerformanceDisplay(densityAltitudeFt, performance) {
    ...
    if (!da) {
        ...
        const tier = daPerformanceTier(performance);
        const isElevated = tier === 'caution' || tier === 'warning';
        const tierEmoji = isElevated ? (tier === 'warning' ? '🚩' : '⚠️') : '';
        return {
            value,
            emoji: tierEmoji,
            className: isElevated ? 'density-altitude-warning' : '',
            title: daPerformanceTooltip(tier, performance),
            ariaLabel: daPerformanceAriaLabel(densityAltitudeFt, tier, performance, distUnit),
        };
    }
...
// NIGHT_MODE_DATA (civil twilight times) is declared in the page bootstrap
...
function displayError(msg) {
    // Log weather errors for diagnostics while preserving a stable empty-field UI
...
function updateWindVisual(weather) {
    ...
    windDetails.innerHTML = `
        ...
        ${(() => {
            ...
            const petalLegendItem = hasActivePetals ? `<span style="display: inline-flex; align-items: center; gap: 0;"><svg width="28" height="14" viewBox="4 4 9 5" style="display: block; flex-shrink: 0;"><g transform="rotate(180 8.5 6.5)"><path d="M4 6 L11.85 4.44 A8 8 0 0 1 11.85 7.56 Z" fill="${colors.windRosePetal}" stroke="${colors.windRosePetalStroke}" stroke-width="1"/><polyline points="10.5,4.87 9.5,6 10.5,7.13" fill="none" stroke="${colors.chevron}" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round"/><polyline points="9,5.15 8,6 9,6.85" fill="none" stroke="${colors.chevron}" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round"/></g></svg><span style="margin-left: 0.15rem;">wind rose petal (${periodLabel})</span></span>` : '';
            return `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (camParam !== null && window.AIRPORT_NAV_DATA?.webcamHistoryEnabled) {
        ...
        const webcams = (AIRPORT_DATA && Array.isArray(AIRPORT_DATA.webcams)) ? AIRPORT_DATA.webcams : [];
...
function updateWebcamTimestamps() {
    for (let camIndex = 0; camIndex < WEBCAM_INITIAL_TIMESTAMPS.length; camIndex++) {
        ...
        if (timestampElem) {
            ...
            const timestampFromAttr = timestampElem.dataset.timestamp;
            const timestampFromCamTs = CAM_TS[camIndex];
            // Parse timestamps to numbers for comparison
            const attrTimestamp = timestampFromAttr ? parseInt(timestampFromAttr) : 0;
            const camTsTimestamp = timestampFromCamTs ? parseInt(timestampFromCamTs) : 0;
            const timestamp = attrTimestamp > 0 ? attrTimestamp : (camTsTimestamp > 0 ? camTsTimestamp : null);

            if (timestamp && timestamp > 0) {
                updateTimestampDisplay(timestampElem, timestamp);
            } else {
                if (!timestampElem.textContent || timestampElem.textContent === '--') {
...
// Initialize CAM_TS with server-side capture timestamps from the page
// bootstrap (EXIF capture time when available, file mtime otherwise)
for (let camIndex = 0; camIndex < WEBCAM_INITIAL_TIMESTAMPS.length; camIndex++) {
    CAM_TS[camIndex] = WEBCAM_INITIAL_TIMESTAMPS[camIndex];
}

// Initialize timestamp displays after DOM is ready
function initializeWebcamTimestamps() {
    for (let camIndex = 0; camIndex < WEBCAM_INITIAL_TIMESTAMPS.length; camIndex++) {
        if (WEBCAM_INITIAL_TIMESTAMPS[camIndex] > 0) {
            const timestampElem = document.getElementById('webcam-timestamp-' + camIndex);
            if (timestampElem) {
                updateTimestampDisplay(timestampElem, WEBCAM_INITIAL_TIMESTAMPS[camIndex]);
            }
        }
    }
}
...
// No-webcam airports skip timestamp display setup and its refresh timer
if (WEBCAM_INITIAL_TIMESTAMPS.length > 0) {
    if (document.readyState === 'loading') {
...
(function() {
    for (let camIndex = 0; camIndex < WEBCAM_INITIAL_TIMESTAMPS.length; camIndex++) {
        (function(idx) {
            const imgEl = document.getElementById('webcam-' + idx);
            if (imgEl) {
                ...
                const timestampDelay = idx === 0 ? 500 : 100;
                if (imgEl.complete && imgEl.naturalHeight !== 0) {
                    ...
                    if (typeof observeWebcamFormat === 'function') {
                        observeWebcamFormat(idx, imgEl);
                    }
                    setTimeout(() => updateWebcamTimestampOnLoad(idx), timestampDelay);
                } else {
        ...
        // Interval floor of 60s is applied server-side in WEBCAM_REFRESH_SECONDS
        setupWebcamRefresh(camIndex, WEBCAM_REFRESH_SECONDS[camIndex]);
    }
